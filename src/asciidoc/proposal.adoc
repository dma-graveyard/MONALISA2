...
===== Use case 1.1: Vessel entering the area

====== Normal use case: CC suggests intended movements for all vessels in A.
[cols="1,5,5"]
|===
| No. | Event | Action

| 1 | CC detects, that V~0~ has crossed the polygon and is now inside A. | CC transmits an addressed message, M~1~, to V~0~, requesting it broadcast its intended movements inside A.footnote:[This is done even if CC already has this information in order to distribute this information to other vessels in A]
| 2| V~0~ receives M~1~. | V~0~ responds by broadcasting message M~2~, which contains the intended movements of V~0~ inside A, including waypoints and estimated time of arrival at each waypoint.
.2+| 3 | M~2~ is received by CC (and likely some of the other vessels in the area). | CC recalculates optimal speeds per vessel.footnote:[with priority to suggest speed changes for V~0~ over other vessels, and fewest possible other vessels, and only for vessels intending to leave A.]
   | *Exception:* M~2~ is never received by CC. | CC retransmits M~1~ to V~0~.
| 4| CC recalculation complete. | CC transmits addressed messages, M~n~, with suggested intended movements to those vessels which (according to the CC calculation) require changes.
.2+| 5| V~n~ receives M~n~. | Navigator is alerted.
| *Exception:* V~n~ is never received by M~n~. | _May lead to special case: CC discovers new suggestions needed._
| 6| Navigator of V~n~ approves suggested intended movement. | V~n~ broadcasts message containing intended movement.
|===
...
====== Special use case: CC discovers new suggestions needed

====== Special use case: V~2~ suggests intended movement for V~0~ in controlled flow management

..

M~2~ is received by CC and some of the other vessels in the area. CC may retransmit M~1~ if it has not received M~2~.
