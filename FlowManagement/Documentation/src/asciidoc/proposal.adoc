...
v0.2, February 4, 2015: 2nd draft for review.
...
|v0.2 |04 FEB 2015 |Niels Bech Nielsen <nbn@logical.dk> | Added Maritime Cloud Components
|===
...
As part of <<ARCH>> vessels are also required to publish their Dynamic Voyage Plan to STCC when it has been changed and safety checked.
...
Given the transmission media there are two sets of use cases depending on the choice of either AIS or MMS based messaging

{set:case:1}

=== Use case (AIS-{counter:case}): Vessel enters the controlled area
...
=== Use case (AIS-{counter:case}): Coordination centre determines new flow management suggestion s needed
...
=== Use case (AIS-{counter:case}): Vessel autonomously broadcasts its tactical voyageplan
...
=== Use case (AIS-{counter:case}): Vessel cancels its tactical voyageplan
...
=== Use case (AIS-{counter:case}): A vessel inquiries about the tactical voyageplan of another vessel
...
=== Use case (MMS-{counter:case}): Requesting a tactical voyage plan
...
=== Use case (MMS-{counter:case}): Vessel receive flow management suggestion
...
== Design of flow management AIS message types and conversations
...
== Design of flow management MMS Message and Endpoint definition
...
|===
| No. | Criteria | Validation | Score
| {counter:step} | Design for multivendor environment | The Maritime Cloud is designed for multivendor environments. | 2
| {counter:step} | Information transfer involving ships must be bandwidth efficient |
The exchanged messages can be long. However, most fields of the tactical voyage plan that were already optional have been omitted from the MSDL. For an STCC this information is available from other sources.| 1
| {counter:step} | Ship-shore interactions must be robust | Messaging in the Maritime Cloud is inherently robus. | 2
| {counter:step} |Ship-shore data IP connections must be initiated from ship | Although MMS provide no direct dependency on IP it does mandate connections to be initated from the ship at any time. | 2
| {counter:step} | Indication of trust | Trust are designed into the Maritime Cloud in the Maritime Identity Register as well as into the MMS caller context | 2
|
|===
...
The reference implementation of the _flow management suggestion_ and _tactical voyageplan exchange_ is available in 

- https://github.com/dma-dk/MONALISA2/tree/master/FlowManagement/Services

==== EPD Reference implementation
...
|IALA |International Association of Lighthouse Authorities |The International Asso
ciation of Marine Aids to Navigation and Lighthouse Authorities is a non-profit organization founded collect and provide nautical expertise and advice.
