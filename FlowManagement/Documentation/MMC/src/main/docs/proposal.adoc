...
:icons: font
:servicedir: ../../../../../Services/
:msdlfile: {servicedir}/src/main/msdl/eu/monalisaproject/flowmanagement.msdl
...
Two endpoint methods are defined in an exchange of a tactical voyage plan.
One endpoint method for requesting a tactical voyage plan and one endpoint for the response.

Such a response may be generated automatically and synchronous, however allowing the response to be sent asynchronously allow the OOW to decide on the best course of
action for the request.

[source]
----
include::{msdlfile}[tags=tvpendpoint]
----

The nature of the asynchronous endpoints allow an exchange of a tactical voyage plan from ship to ship as well as ship to shore. 

// Provide an illustration?

In a single ship to shore scenario the ship is expected to implement the behaviour behind a request method and the shore is expected to implement the behaviour behind the response method. 

In a ship to ship scenario ships can request a tactical voyage plan from other ships and would need to implement the reponse method as well.

A shore station cannot logically emit tactical voyage plan and may ignore a request or simply provide an empty response.
...
[source]
----
include::{msdlfile}[tags=flowendpoint]
----
