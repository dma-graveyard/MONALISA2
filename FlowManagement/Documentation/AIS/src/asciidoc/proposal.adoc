...
Waypoints are denoted WP~0~, WP~1~, WP~i~,..., WP~n~ and are navigated in sequence. WP~0~ is the _active waypoint_ currently steered towards. WP~i~, where i ≥ 1, is called _following waypoints_.
...
.4+|ETA of WP~0~ |4 |UTC month +
Integer value +
Values outside the range 1-12 are illegal and must not be used.
|5 |UTC day +
Integer value +
Values outside the range 1-31 are illegal and must not be used.
|5 |UTC hour +
Integer value +
...
|Active waypoint, longitude |28 | Longitude in 1/10,000 min, ±180 degrees as per 2's complement (East = positive, West = negative).
|Active waypoint, latitude |27 | Latitude in 1/10,000 min, ±90 degrees as per 2's complement (North = positive, South = negative).
|Next waypoints with ETA | n × 63 | Variable no. of planned waypoints and ETA's. +
n ∈ {0..14} +
 See table 2
|*Total* | *55-1019*
|55 bits for cancellation of tactical voyageplan. +
137-1019 bits for tactical voyageplan with 0..14 following waypoints.
 +
e.g. 137 bits for n=0, 200 bits for n=1, 1019 bits for n=14. +
 +
*Occupies X – X slots.* +
1 - 4 waypoints = ? slots +
5 - 8 waypoints = ? slots +
9 - 12 waypoints = ? slots +
13 – 14 waypoints = ? slots
|===

Table 2. Waypoints with relative ETA's.
[cols="4,2,12"]
|===
|Parameter |No. of bits |Description

|Longitude of WP~n~ |28 |Longitude in 1/10,000 min, ±180 degrees as per 2's complement (East = positive, West = negative).
|Latitude of WP~n~  |27 |Latitude in 1/10,000 min, ±90 degrees as per 2's complement (North = positive, South = negative).
|Relative ETA at WP~n~ | 8 | Relative ETA from previous waypoint; measured in minutes. +
Integer value; [0-255]. +
...
=== Tactical voyageplan inquiry
Proposed specification of an AIS message to implement message "Tactical voyage plan inquiry":

====

The message can be transmitted by vessels or shore-based coordination centres.

Retransmission -- except in the sense of missing protocol acknowledgement as per <<AISSPEC5>>, Annex 8 §3.5 -- is not applicable.
...
*DAC*:  219 +
*FI*:  3 +
*Used by*: DMA, MONALISA 2 +
*Number of Slots (max)*:  1 +
*Reporting rate*:  See "transmission triggers" above +
*How portrayed*: -.
